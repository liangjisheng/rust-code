...
// 通过开头我们知道，自引用最麻烦的就是创建引用的同时，值的所有权会被转移，而通过 Pin 就可以
// 很好的防止这一点
...
// 上面的代码也非常清晰，虽然使用了 unsafe，其实更多的是无奈之举，跟之前的 unsafe
// 实现完全不可同日而语。
// 其实 Pin 在这里并没有魔法，它也并不是实现自引用类型的主要原因，最关键的还是里面的
// 裸指针的使用，而 Pin 起到的作用就是确保我们的值不会被移走，否则指针就会指向一个错
// 误的地址！
